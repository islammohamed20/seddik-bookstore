...
@section('page-title', 'إضافة طريقة شحن جديدة لمنطقة: ' . $shippingZone->name_ar)

@section('content')
<div class="max-w-4xl" x-data="{ type: '{{ old('type', 'flat_rate') }}' }">
    <form action="{{ route('admin.shipping-methods.store') }}" method="POST" class="space-y-6" id="main-form">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="type" id="type" x-model="type" required
                        ...
                        <option value="pickup">الاستلام من المتجر</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div x-show="['flat_rate', 'weight_based'].includes(type)">
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" name="free_shipping_threshold" id="free_shipping_threshold" value="{{ old('free_shipping_threshold') }}"
                               class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 pl-12">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <span class="text-gray-500 sm:text-sm">ج.م</span>
                        </div>
                ...
                <div x-show="type === 'weight_based'" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <div>
                        ...
                        <input type="number" step="0.01" name="min_weight" id="min_weight" value="{{ old('min_weight') }}"
                               class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label for="max_weight" class="block text-sm font-medium text-gray-700 mb-1">أقصى وزن (كجم)</label>
                        <input type="number" step="0.01" name="max_weight" id="max_weight" value="{{ old('max_weight') }}"
                               class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                </div>
        ...
        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('admin.shipping-zones.show', $shippingZone->id) }}" class="text-gray-600 hover:text-gray-900">إلغاء</a>
